...
_Cvolution_kernel = kernel_loop + '''
extern "C"
__global__ void Cvolution_forward_kernel(
...
_Cvolution_kernel_backward_grad_input = kernel_loop + '''
extern "C"
__global__ void Cvolution_backward_grad_input_kernel(
...
_Cvolution_kernel_backward_grad_weight = kernel_loop + '''
extern "C"
__global__ void Cvolution_backward_grad_weight_kernel(
...
class _Cvolution(Function):
    ...
    def forward(ctx, input, weight, stride, padding, dilation):
        ...
        with torch.cuda.device_of(input):
            f = load_kernel('Cvolution_forward_kernel', _Cvolution_kernel, Dtype=Dtype(input), nthreads=n,
    ...
    def backward(ctx, grad_output):
        ...
        with torch.cuda.device_of(input):
            if ctx.needs_input_grad[0]:
                ...
                f = load_kernel('Cvolution_backward_grad_input_kernel',
                                _Cvolution_kernel_backward_grad_input, **opt)
                f(block=(CUDA_NUM_THREADS, 1, 1),
                  grid=(GET_BLOCKS(n), 1, 1),
            ...
            if ctx.needs_input_grad[1]:
                ...
                f = load_kernel('Cvolution_backward_grad_weight_kernel',
                                _Cvolution_kernel_backward_grad_weight, **opt)
                f(block=(CUDA_NUM_THREADS, 1, 1),
                  grid=(GET_BLOCKS(n), 1, 1),
...
def __Cvolution_cuda(input, weight, bias=None, stride=1, padding=0, dilation=1):
    assert input.size(0) == weight.size(0)
    assert input.size(-2) // stride == weight.size(-2)
    assert input.size(-1) // stride == weight.size(-1)
    if input.is_cuda:
        out = _Cvolution.apply(input, weight, _pair(stride), _pair(padding), _pair(dilation))
        if bias is not None:
...
class Cvolution(nn.Module):

    def __init__(self,
                 channels,
                 kernel_size,
                 stride):
        super(Cvolution, self).__init__()
        self.kernel_size = kernel_size
        self.stride = stride
        self.channels = channels  # 256
        reduction_ratio = 4
        self.group_channels = 16
        self.groups = self.channels // self.group_channels  # 16

        self.conv1 = nn.Sequential(
            nn.Conv2d(channels, channels // reduction_ratio, kernel_size=1),  # 64
            nn.BatchNorm2d(channels // reduction_ratio),
            nn.ReLU())

        self.conv2 = nn.Sequential(
            nn.Conv2d(channels // reduction_ratio, kernel_size ** 2 * self.groups, kernel_size=1, stride=1))  # 144
        if stride > 1:
            self.avgpool = nn.AvgPool2d(stride, stride)
        #        self.unfold = nn.Unfold(kernel_size, 1, (kernel_size-1)//2, stride)

        self.conv_q = nn.Conv2d(channels, channels, kernel_size=3, padding=kernel_size // 2, groups=16, bias=False)
        self.avg_pool_c = nn.AdaptiveAvgPool2d(1)

    def forward(self, x):  # (3,256,7,7)

        g_x = self.conv_q(x)
        avg_x = self.avg_pool_c(g_x)

        weight = self.conv2(self.conv1(x if self.stride == 1 else self.avgpool(x)))
        b, c, h, w = weight.shape
        weight = weight.view(b, self.groups, self.kernel_size, self.kernel_size, h, w)
        out = _Cvolution((x * avg_x.expand_as(x)), weight, stride=self.stride,padding=(self.kernel_size - 1) // 2)
        return out